...
- Администраторы провайдеров и другие пользователи с правами доступа к админке могут войти:
  - Через стандартную форму входа (email + пароль), если у пользователя есть пароль
  - Через кнопку "Войти через Google" на странице входа в админку, если пользователь регистрировался через Google
- После входа пользователь получает доступ к функциям согласно своей роли:
  - Администратор провайдера - доступ только к своей организации
  - Системный администратор - полный доступ ко всем функциям
  - Биллинг-менеджер - доступ к назначенным организациям
...
3. Выбирает категории услуг из каталога (обязательно минимум 1)
4. Загружает документы (лицензии, сертификаты) в случае, если учреждение предоставляет услуги, которые требуют лицензирования / сертификации
5. Система проверяет корректность данных и выбранных категорий
6. После модерации учреждение активируется
7. После активации автоматически устанавливаются выбранные категории как доступные для учреждения
8. Администратор провайдера получает ограниченный доступ только к выбранным категориям
...
**Реквизиты организации (Provider):**
- **Обязательные реквизиты (заполняются при подаче заявки):**
  - Название организации (уникальное)
  - Юридический адрес организации (для документов, договоров)
  - Email организации (уникальный на уровне организации)
  - Телефон организации (уникальный на уровне организации)
  - Категории услуг уровня 0, которые организация может предоставлять
  - Статус активности организации

- **Обязательные реквизиты для аппрува (заполняются биллинг-менеджерами):**
  - ИНН / налоговый номер (уникальный, обязателен для аппрува)
  - Регистрационный номер (уникальный, обязателен для аппрува)

- **Опциональные реквизиты:**
  - Сайт организации
  - Логотип организации
...
1. Системный администратор просматривает заявку учреждения (ProviderForm) в админке Django
2. Системный администратор использует действие "Одобрить и назначить биллинг-менеджера"
3. В форме выбора системный администратор выбирает биллинг-менеджера из списка доступных менеджеров по биллингу
4. При одобрении заявки автоматически:
   - Создается организация (Provider) с данными из заявки (название, юридический адрес, контакты)
   - Создается связь ProviderAdmin (связь создателя заявки с организацией)
   - Назначается роль provider_admin создателю заявки
   - Создается связь BillingManagerProvider (связь выбранного биллинг-менеджера с организацией)
   - Создается событие BillingManagerEvent типа 'assigned' для истории назначений
5. Статус заявки меняется на 'approved'
6. После активации провайдера администратор провайдера получает письмо с уведомлением и ссылкой на админку
7. Администратор провайдера создает локации вручную через административный интерфейс
8. Администратор провайдера имеет доступ только к своей организации и не может создавать локации для других организаций

**Заполнение реквизитов организации:**
- После создания организации провайдер получает статус `pending` (ожидает заполнения реквизитов)
- Назначенный биллинг-менеджер заполняет обязательные реквизиты:
  - ИНН / налоговый номер (уникальный, обязателен для активации)
  - Регистрационный номер (уникальный, обязателен для активации)
  - Вебсайт (опционально)
- Биллинг-менеджер заполняет реквизиты в админке Django в разделе Provider (Organization Details)
- После заполнения реквизитов биллинг-менеджер может изменить статус провайдера:
  - `pending` → `activation_required` (готов к активации админом)
  - `pending` → `rejected` (отклонен биллинг-менеджером)
  - `activation_required` → `pending` (вернуть на доработку, если найдена ошибка)
  - `rejected` → `pending` (вернуть на рассмотрение)
...
**Активация провайдера и создание учетной записи администратора:**

При активации провайдера (изменение статуса на 'active') система выполняет следующие действия:

1. **Определение администратора провайдера:**
   - Администратором провайдера является автор заявки (`ProviderForm.created_by`)
   - Автору заявки автоматически назначается роль `provider_admin`
   - Создается связь `ProviderAdmin` между пользователем и провайдером

2. **Отправка писем об активации:**
   - Письмо отправляется **двум получателям**:
     - **Автору заявки** (администратору провайдера) - на его email
     - **На email провайдера** (из `ProviderForm.provider_email`) - если он отличается от email автора
   - В письме содержится:
     - Информация об активации провайдера
     - Ссылка на админ-панель
     - Инструкция по входу (email/пароль или Google)
     - Контакты billing_manager 
     - Ссылка на подробную инструкцию на фронте (`/provider-setup-guide`)

3. **Права доступа:**
   - Только автор заявки имеет права администратора провайдера
   - Пользователь с email провайдера (если отличается от автора) получает только информационное письмо
   - Для доступа к админ-панели необходимо иметь роль `provider_admin` и права `is_staff=True`
- При одобрении заявки (`ProviderForm.status = 'approved'`):
  - Создается организация (Provider) с данными из заявки
  - Создается связь `ProviderAdmin` (связь создателя заявки `ProviderForm.created_by` с организацией)
  - Назначается роль `provider_admin` создателю заявки
  - Учетная запись НЕ создается (создатель заявки уже имеет учетную запись)
  - Письмо НЕ отправляется (провайдер еще не активирован)
- При активации провайдера (`activation_status = 'active'`):
  - Системный администратор переводит статус провайдера из `activation_required` в `active`
  - Автоматически отправляется письмо создателю заявки (теперь администратору провайдера) на его email (`ProviderForm.created_by.email`)
  - Содержание письма:
    - Стандартное приветствие
    - Уведомление об активации провайдера с названием организации
    - Адрес административной панели (ссылка на админку)
    - Инструкции по входу:
      - Если у пользователя есть пароль: "Ваш логин: {email}, используйте ваш существующий пароль"
      - Если у пользователя нет пароля (регистрация через Google): "Войдите через Google, используя ваш аккаунт {email}"
- Администратор провайдера может войти в админку:
  - Через стандартную форму входа (email + пароль), если у него есть пароль
  - Через кнопку "Войти через Google" на странице входа в админку, если он регистрировался через Google
  - После входа администратор провайдера имеет доступ только к своей организации и может управлять локациями, услугами, сотрудниками и расписаниями
...
1. Администратор организации входит в систему после получения учетных данных
2. Переходит в раздел управления локациями
3. Нажимает "Добавить локацию"
4. Заполняет обязательные поля:
   - Название локации
   - Адрес локации (геокодируется автоматически)
   - Email локации
   - Телефон локации
...
**Административный интерфейс:**
- Вкладка "Locations" в ProviderAdmin
...
- Все таблицы (расписания, услуги, сотрудники) работают с локациями, а не с организациями
- Биллинг-менеджеры работают с организациями для договоров и биллинга
...
6. Биллинг-менеджер заполняет обязательные реквизиты организации (ИНН, регистрационный номер) после назначения
