...
- **Реальные bookable slots:** В поиске и в выдаче слотов показываются только реально бронируемые окна с учётом расписания филиала, расписания и активности сотрудника, длительности услуги, существующих booking conflicts, pet owner conflicts, escort conflicts и времени на перемещение между визитами.
- **Routing-only проверка логистики:** Проверка возможности доехать между соседними визитами переведена на routing integration без гео-эвристического fallback. Если routing API недоступен, availability/create возвращают явную ошибку вместо "приближённого" решения.
