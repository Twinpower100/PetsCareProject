...
def get_public_site_base_url() -> str:
    ...
    In production this can be managed via Django's Sites admin. Environment
    variables remain the fallback and the default source for local development.
    """
    if getattr(settings, 'SITE_URLS_USE_DJANGO_SITE', False):
        try:
            site_url = _url_from_domain(Site.objects.get_current().domain)
            if site_url:
                return site_url
        except Exception:
            pass
