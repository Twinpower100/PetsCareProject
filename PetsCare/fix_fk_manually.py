"""
Скрипт для ручного исправления внешних ключей в базе данных.
"""
import os
import django
...
def fix_foreign_keys():
    """Исправляет внешние ключи, ссылающиеся на auth_user вместо users_user."""
    with connection.cursor() as cursor:
        # Проверяем все внешние ключи в billing, которые ссылаются на auth_user
        cursor.execute("""
            SELECT 
        ...
        for constraint_name, table_name, referenced_table in constraints:
            ...
            try:
                ...
                print(f"  ⚠ Ошибка при удалении {constraint_name}: {e}")
                continue
            ...
            try:
                ...
                print(f"  ⚠ Ошибка при создании {new_constraint_name}: {e}")
