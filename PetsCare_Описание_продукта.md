...
Описание:
Пользователь, у которого есть хотя бы один питомец. В системе реализована модель совместного владения через роли "Основной владелец" и "Совладелец".
Первый владелец питомца (лицо, создавшее профиль питомца), назначается основным владельцем. У питомца всегда строго один основной владелец.

Возможности разделены по ролям:
•	Совладелец (Co-owner): может просматривать профиль, редактировать данные питомца, записывать на приемы, добавлять медицинские записи и документы, пользоваться услугами передержки. Совладелец может самостоятельно отказаться от своих прав.
•	Основной владелец (Main Owner): имеет все права Совладельца, а также эксклюзивные права:
  - Деактивировать (soft-delete) питомца (питомцы физически не удаляются из БД).
  - Отправлять инвайты (приглашения) другим пользователям стать совладельцами.
  - Удалять совладельцев.
  - Передать свои полномочия основного владельца другому лицу (при этом передающий автоматически становится обычным совладельцем).
•	Поиск и выбор услуг:
